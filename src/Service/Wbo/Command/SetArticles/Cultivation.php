...
class Cultivation
{
    ...
    public function execute(Article $article, array &$productData): void
    {
        ...
        if (empty($article->getWineDetails()->getCultivation())) {
            return;
        }

        $property = $this->getProperty();
        if (!$property) {
            $this->logger->debug('Could not set cultivation property, there is no valid property configured, please configure property id for attribution cultivation');
            return;
        }

        $option = $this->getOption($property->getId(), $article->getWineDetails()->getCultivation());
        if (!$option) {
            $option = $this->createOption($property->getId(), $article->getWineDetails()->getCultivation());
        }
        if (!$option) {
            return;
        }
        ...
        $productData['properties'][] = [
            'id' => $option->getId(),
            'name' => $article->getWineDetails()->getCultivation(),
    ...
    {
        $propertyId = $this->wboConfig->get(WboConfig::PROPERTY_CULTIVATION_ID);
        if (!$propertyId) {
            return null;
        }
    ...
    {
        $canAddNewOption = $this->wboConfig->get(WboConfig::PROPERTY_CULTIVATIONA_AUTO_ADD);
        if (!$canAddNewOption) {
            $this->logger->debug('could not create new cultivation option, automaticly add is disabled');
            return null;
        }
        ...
        $collection = $this->propertyGroupOptionRepository->search(new Criteria([$id]),$this->context);

        if (!$collection->count()) {
            $this->logger->debug('could not create new cultivation option');
            return null;
        }
